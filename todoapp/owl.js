...
function setupSyntheticEvent(evName, eventKey, capture = false) {
    ...
    document.addEventListener(
        evName,
        (event) => nativeToSyntheticEvent(eventKey, event),
        {
            capture,
        }
    )
...
class VText$1 extends VSimpleNode {
    mount(parent, afterNode) {
        this.mountNode(
            document.createTextNode(toText(this.text)),
            parent,
            afterNode
        )
    }
...
class VComment extends VSimpleNode {
    mount(parent, afterNode) {
        this.mountNode(
            document.createComment(toText(this.text)),
            parent,
            afterNode
        )
    }
...
function buildContext(tree, ctx, fromIdx) {
    if (!ctx) {
        const children = new Array(
            tree.info.filter((v) => v.type === 'child').length
        )
        ctx = {
            collectors: [],
            locations: [],
            ...
            if (childN) {
                ...
                for (let i = 0; i < childN; i++) {
                    ...
                    if (child) {
                        ...
                        const afterNode = loc.afterRefIdx
                            ? refs[loc.afterRefIdx]
                            : null
        ...
        if (startIdx1 <= endIdx1 || startIdx2 <= endIdx2) {
            if (startIdx1 > endIdx1) {
                ...
                const anchor = nextChild
                    ? cFirstNode.call(nextChild) || null
                    : _anchor
    ...
    render() {
        ...
        while (current) {
            if (current.fiber) {
                ...
                if (
                    root.counter === 0 &&
                    prev.parentKey in current.fiber.childrenMap
                ) {
                    current = root.node
                } else {
    ...
    complete() {
        ...
        try {
            ...
            if (node.bdom) {
                ...
                node.bdom = this.bdom
                if (
                    this.position === 'last-child' ||
                    this.target.childNodes.length === 0
                ) {
...
const SUPPORTED_RAW_TYPES = new Set([
    'Object',
    'Array',
    'Set',
    'Map',
...
const rawTypeToFuncHandlers = {
    Set: (target, callback) => ({
        ...
        [Symbol.iterator]: makeIteratorObserver(
            Symbol.iterator,
            target,
            callback
        ),
...
function collectionsProxyHandler(target, callback, targetRawType) {
    ...
    // property is read.
    const specialHandlers = rawTypeToFuncHandlers[targetRawType](
        target,
        callback
    )
...
class ComponentNode {
    ...
    async updateAndRender(props, parentFiber) {
        ...
        const prom = Promise.all(
            this.willUpdateProps.map((f) => f.call(component, props))
        )
        await prom
        if (fiber !== this.fiber) {
            return
...
function wrapError(fn, hookName) {
    ...
    const timeoutError = new OwlError(
        `${hookName}'s promise hasn't resolved after 3 seconds`
    )
    const node = getCurrent()
    return (...args) => {
        const onError = (cause) => {
        ...
        try {
            ...
            if (result instanceof Promise) {
                if (
                    hookName === 'onWillStart' ||
                    hookName === 'onWillUpdateProps'
                ) {
                    const fiber = node.fiber
                    Promise.race([
                        result.catch(() => {}),
...
function onWillUpdateProps(fn) {
    ...
    node.willUpdateProps.push(
        decorate(fn.bind(node.component), 'onWillUpdateProps')
    )
}
...
function isOptional(t) {
    return typeof t === 'object' && 'optional' in t
        ? t.optional || false
        : false
}
...
function validateSchema(obj, schema) {
    ...
    for (let key in schema) {
        ...
        if (key !== '*' && !isOptional(spec) && !(key in obj)) {
            ...
            const isAny =
                spec === '*' ||
                (isObj && 'type' in spec ? spec.type === '*' : isObj)
...
function validateType(key, value, descr) {
    if (value === undefined) {
        return isOptional(descr)
            ? null
            : `'${key}' is undefined (should be a ${describe(descr)})`
    } else if (isBaseType(descr)) {
        return validateBaseType(key, value, descr)
    } else if (isValueType(descr)) {
        return value === descr.value
            ? null
            : `'${key}' is not equal to '${descr.value}'`
...
function callSlot(ctx, parent, key, name, dynamic, extra, defaultContent) {
    ...
    const slotBDom = __render
        ? __render.call(__ctx.__owl__.component, slotScope, parent, key)
        : null
    if (defaultContent) {
        ...
        if (slotBDom) {
            ...
            child2 = defaultContent.call(
                ctx.__owl__.component,
                ctx,
                parent,
                key
            )
...
function validateProps(name, props, parent) {
    ...
    if (!schema) {
        if (parent.__owl__.app.warnIfNoStaticProps) {
            console.warn(
                `Component '${ComponentClass.name}' does not have a static props description`
            )
        }
        return
    }
    ...
    if (errors.length) {
        throw new OwlError(
            `Invalid props for component '${ComponentClass.name}': ` +
                errors.join(', ')
        )
    }
}
...
class TemplateSet {
    ...
    addTemplate(name, template) {
        if (name in this.rawTemplates) {
            ...
            const newAsString =
                typeof template === 'string' ? template : template.outerHTML
            if (currentAsString === newAsString) {
                return
            }
            throw new OwlError(
                `Template ${name} already defined with different content`
            )
        }
    ...
    getTemplate(name) {
        if (!(name in this.templates)) {
            ...
            if (rawTemplate === undefined) {
                ...
                try {
                    const componentName =
                        getCurrent().component.constructor.name
...
function compileExprToArray(expr) {
    ...
    while (i < tokens.length) {
        ...
        let isVar =
            token.type === 'SYMBOL' && !RESERVED_WORDS.includes(token.value)
...
class BlockDescription {
    ...
    generateExpr(expr) {
        if (this.type === 'block') {
            ...
            if (hasChildren) {
                params +=
                    ', [' + this.children.map((c) => c.varName).join(', ') + ']'
            }
...
class CodeGenerator {
            ...
            for (let block of this.blocks) {
                if (block.dom) {
                    ...
                    if (block.dynamicTagName) {
                        ...
                        mainCode.push(
                            `let ${block.blockName} = createBlock(\`${xmlString}\`);`
                        )
                    }
                }
            }
    ...
    insertBlock(expression, block, ctx) {
        ...
        if (block.parentVar) {
            ...
            this.addLine(
                `${block.parentVar}[${ctx.index}] = withKey(${blockExpr}, ${keyArg});`
            )
            return
        }
        if (tKeyExpr) {
    ...
    compileTDomNode(ast, ctx) {
        let { block, forceNewBlock } = ctx
        const isNewBlock =
            !block || forceNewBlock || ast.dynamicTag !== null || ast.ns
        ...
        for (let key in ast.attrs) {
            ...
            if (attrName === 'value' && ctx.tModelSelectedExpr) {
                let selectedId = block.insertData(
                    `${ctx.tModelSelectedExpr} === ${expr}`,
                    'attr'
                )
        ...
        if (ast.ref) {
            ...
            if (isDynamic) {
                const str = replaceDynamicParts(ast.ref, (expr) =>
                    this.captureExpression(expr, true)
                )
        ...
        if (ast.model) {
            ...
            if (specialInitTargetAttr) {
                idx = block.insertData(
                    `${fullExpression} === '${attrs[targetAttr]}'`,
                    'attr'
                )
            ...
            if (block.children.length && block.hasDynamicChildren) {
                ...
                for (let i = codeIdx; i < code.length; i++) {
                    if (
                        code[i]
                            .trimStart()
                            .startsWith(`const ${current.varName} `)
                    ) {
                        code[i] = code[i].replace(
                            `const ${current.varName}`,
                            current.varName
                        )
                ...
                this.addLine(
                    `let ${block.children.map((c) => c.varName)};`,
                    codeIdx
                )
            }
        }
    ...
    compileTEsc(ast, ctx) {
        ...
        if (!block || forceNewBlock) {
            block = this.createBlock(block, 'text', ctx)
            this.insertBlock(`text(${expr})`, block, {
                ...ctx,
                forceNewBlock: forceNewBlock && !block,
            })
        } else {
    ...
    compileTIfBranch(content, block, ctx) {
        ...
        this.compileAST(
            content,
            createContext(ctx, { block, index: ctx.index })
        )
    ...
    compileTForeach(ast, ctx) {
        ...
        this.define(
            `[${keys}, ${vals}, ${l}, ${c}]`,
            `prepareList(${compileExpr(ast.collection)});`
        )
        ...
        this.addLine(
            `for (let ${loopVar} = 0; ${loopVar} < ${l}; ${loopVar}++) {`
        )
        this.target.indentLevel++
        ...
        if (!ast.hasNoLast) {
            this.addLine(
                `ctx[\`${ast.elem}_last\`] = ${loopVar} === ${vals}.length - 1;`
            )
        }
        if (!ast.hasNoIndex) {
        ...
        this.define(
            `key${this.target.loopLevel}`,
            ast.key ? compileExpr(ast.key) : loopVar
        )
        if (this.dev) {
        ...
        if (ast.memo) {
            this.addLine(
                `nextCache[key${this.target.loopLevel}] = Object.assign(${c}[${loopVar}], {memo: memo${id}});`
            )
        }
        this.target.indentLevel--
        this.target.loopLevel--
    ...
    compileTCall(ast, ctx) {
        ...
        const subTemplate = isDynamic
            ? interpolate(ast.name)
            : '`' + ast.name + '`'
        if (block) {
            if (!forceNewBlock) {
                this.insertAnchor(block)
        ...
        if (isDynamic) {
            ...
            block = this.createBlock(block, 'multi', ctx)
            this.insertBlock(
                `call(this, ${templateVar}, ${ctxVar}, node, ${key})`,
                block,
                {
                    ...ctx,
                    forceNewBlock: !block,
    ...
    compileTCallBlock(ast, ctx) {
        ...
        block = this.createBlock(block, 'multi', ctx)
        this.insertBlock(compileExpr(ast.name), block, {
            ...ctx,
            forceNewBlock: !block,
        })
    }
    compileTSet(ast, ctx) {
        ...
        if (ast.body) {
            ...
            this.addLine(
                `setContextValue(${ctx.ctxVar || 'ctx'}, "${
                    ast.name
                }", ${value});`
            )
        }
    ...
    compileComponent(ast, ctx) {
        ...
        if (ast.slots) {
            ...
            for (let slotName in ast.slots) {
                ...
                const params = []
                if (slotAst.content) {
                    const name = this.compileInNewTarget(
                        'slot',
                        slotAst.content,
                        ctx,
                        slotAst.on
                ...
                if (ast.slots[slotName].attrs) {
                    params.push(
                        ...this.formatPropObject(ast.slots[slotName].attrs)
                    )
                }
        ...
        if (slotDef && (ast.dynamicProps || hasSlotsProp)) {
            this.helpers.add('markRaw')
            this.addLine(
                `${propVar}.slots = markRaw(Object.assign(${slotDef}, ${propVar}.slots))`
            )
        }
        // cmap key
        ...
        let blockExpr = `${id}(${propString}, ${keyArg}, node, this, ${
            ast.isDynamic ? expr : null
        })`
        if (ast.isDynamic) {
    ...
    compileTTranslation(ast, ctx) {
        if (ast.content) {
            this.compileAST(
                ast.content,
                Object.assign({}, ctx, { translate: false })
            )
        }
    }
    ...
    for (let attr of nodeAttrsNames) {
        ...
        if (attr.startsWith('t-on')) {
            ...
            if (hasDotAtTheEnd.test(value)) {
                ...
                throw new OwlError(
                    `Invalid t-model expression: "${value}" (it should be assignable)`
                )
            }
...
function parseTSetNode(node, ctx) {
    ...
    const defaultValue =
        node.innerHTML === node.textContent ? node.textContent || null : null
    let body = null
    ...
    for (let name of node.getAttributeNames()) {
        ...
        if (name.startsWith('t-')) {
            if (name.startsWith('t-on-')) {
                ...
                const message = directiveErrorMap.get(
                    name.split('-').slice(0, 2).join('-')
                )
                throw new OwlError(
                    message || `unsupported directive on Component: ${name}`
                )
            }
        } else {
            props = props || {}
    ...
    for (let i = 0, ilen = tbranch.length; i < ilen; i++) {
        ...
        if (prevElem && (pattr('t-if') || pattr('t-elif'))) {
            ...
            while ((textNode = node.previousSibling) !== prevElem) {
                if (
                    textNode.nodeValue.trim().length &&
                    textNode.nodeType !== 8
                ) {
                    throw new OwlError(
    ...
    const hasSafeContext =
        template instanceof Node
            ? !(template instanceof Element) ||
              template.querySelector('[t-set], [t-call]') === null
...
const mainEventHandler = (data, ev, currentTarget) => {
    ...
    if (Object.hasOwnProperty.call(data, 0)) {
        ...
        if (typeof handler !== 'function') {
            throw new OwlError(
                `Invalid handler (expected a function, received: '${handler}')`
            )
        }
...
class App extends TemplateSet {
    constructor(Root, config = {}) {
        ...
        this.env = Object.freeze(
            Object.create(Object.getPrototypeOf(env), descrs)
        )
    ...
    createComponent(
        ...
        return (props, key, ctx, parent, C) => {
            ...
            if (node) {
                if (
                    arePropsDifferent(node.props, props) ||
                    parentFiber.deep ||
                    node.forceNextRender
                ) {
...
function useExternalListener(target, eventName, handler, eventParams) {
    ...
    onWillUnmount(() =>
        target.removeEventListener(eventName, boundHandler, eventParams)
    )
}
...
__info__.date = '2022-07-22T07:43:44.044Z'
