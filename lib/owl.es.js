...
function validateSchema(obj, schema) {
    ...
    for (let key in schema) {
        ...
        if (key !== '*' && !isOptional(spec) && !(key in obj)) {
            ...
            const isAny =
                spec === '*' || (isObj && 'type' in spec ? spec.type === '*' : isObj);
...
class TemplateSet {
    ...
    getTemplate(name) {
        if (!(name in this.templates)) {
            ...
            const isFn =
                typeof rawTemplate === 'function' && !(rawTemplate instanceof Element);
...
class CodeGenerator {
        ...
        if (isNewBlock) {
            if (block.hasDynamicChildren) {
                if (block.children.length) {
                    ...
                    for (let i = codeIdx; i < code.length; i++) {
                        if (
                            code[i].trimStart().startsWith(`const ${current.varName} `)
                        ) {
                            code[i] = code[i].replace(
                                `const ${current.varName}`,
                                current.varName
    ...
    compileTSet(ast, ctx) {
        ...
        if (ast.body) {
            ...
            this.addLine(
                `setContextValue(${ctx.ctxVar || 'ctx'}, "${ast.name}", ${value});`
            );
        }
    }
